...
def analyze(model, testData):  # A subroutine to analyze a list of domains for their respective entropy and classify them as valid or DGA
	...
	confusion = getConfusion(flags)
	print(confusion)
	plot(lengths, measures, flags, testData)


def getConfusion(flags):  # This will compare the predicted classes to the correct classes and produce a confusion matrix for use in tuning
	answers = [True, True, True, True, True, False, True, True, True, True, True, False, True, True, False, True, True, True, True, False, True, True, False, False, True, True]
	matrix = [0, 0, 0, 0]
	for i in range(0, len(flags)):
		...
		if guess and answer:
			matrix[0] = matrix[0] + 1
		elif guess and not answer:
			matrix[1] += 1
		elif answer and not guess:
			matrix[2] += 1
		else:
			matrix[3] += 1
	return matrix
...
def getGramTotal(model,
				 gram):  # A subroutine to calculate the weight of a given gram branch to act as the entropy denominator
	total = 0
	branch = model[gram]
	for key in branch:
		total += branch[key]
